...
public class AnalysisService {
    ...
    private final AutoBriefingWriter autoBriefingWriter;
    ...
    public AnalysisService(
            ...
            AiBriefingService aiBriefingService, AutoBriefingWriter autoBriefingWriter,
        ...
        this.autoBriefingWriter = autoBriefingWriter;
    ...
    private void triggerMultiAgentBriefingSafely(
        ...
        try {
            ...
            // 평가(generate→procurement_risk_assessments)와 브리핑(recordAutoGenerated→ai_briefings)을
            // 한 트랜잭션으로 원자 저장한다(AutoBriefingWriter). 둘 다 JdbcTemplate 삽입이라 예전엔
            // 각각 즉시 auto-commit돼 한쪽만 남는 반쪽 상태가 생겼다("브리핑은 있는데 평가 없음" 등,
            // 실측 2건 → 대시보드 KPI 불일치의 원인). 이제 하나라도 실패하면 함께 롤백된다.
            autoBriefingWriter.generateAndRecord(
                    request, analysis, erpMaterialId,
                    erpContext.resolvedErpSupplierId(), materialCategory);
        } catch (RuntimeException exception) {
