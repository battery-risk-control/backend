...
public final class MaterialRiskDto {
     ...
     * "AI 브리핑 생성" 응답.
     *
     * <p>{@code sourceAnalysisId} 이하 4개는 <b>외부신호로 무엇을 썼는지</b>를 밝히는 필드다.
     * 이 화면에는 뉴스가 없어서 같은 자재 대분류의 저장된 최신 분석을 끌어다 쓰는데, 그 출처를
     * 감추면 "왜 이 점수가 나왔는지"를 화면에서 되짚을 수 없다.
     */
    public record Briefing(
            @Schema(description = "저장된 procurement_risk_assessments 행의 id. 재조회에 쓴다.")
            UUID assessmentId,

            String erpMaterialId,
            String materialName,

            @Schema(description = "외부신호로 사용한 analyses 행의 id")
            UUID sourceAnalysisId,

            @Schema(description = "외부신호로 사용한 기사 제목")
            String sourceHeadline,

            @Schema(example = "CD") String sourceCountryCode,

            @Schema(example = "60.0", description = "그 분석의 외부신호 점수(0~100)")
            Double sourceExternalSignalScore,

            /**
             * ERP·계약 노드까지 실제로 실행돼 종합 점수가 나왔는지.
             *
             * <p><b>false면 risk_level·risk_score를 등급으로 읽으면 안 된다.</b> LangGraph는 KG 게이트에서
             * 뉴스의 자재·국가와 매칭되는 공급망 경로를 못 찾으면 ERP·계약 노드를 통째로 건너뛰고
             * 조기 종료하는데, 그때도 점수 0 · 등급 NORMAL로 응답이 나간다. 그 0을 "판정 결과 정상"으로
             * 읽으면 <b>평가하지 못한 것을 안전하다고 표기</b>하게 된다 — 화면은 대신 risk_reasons에 담긴
             * 조기 종료 사유를 보여줘야 한다. RiskMonitoringDto.ProcurementRisk.completed와 같은 판정이다.
             */
            @Schema(description = "ERP·계약 노드까지 실제로 돌아 종합 점수가 나왔는지. false면 점수가 의미 없다.")
            boolean completed,

            @Schema(example = "CRITICAL", description = "CRITICAL/WARNING/NORMAL. completed=true일 때만 등급의 근거")
            String procurementRiskLevel,

            @Schema(example = "78") int procurementRiskScore,
            List<String> riskReasons,
            List<String> recommendedActions,

            @Schema(description = "브리핑 본문")
            String briefing,
