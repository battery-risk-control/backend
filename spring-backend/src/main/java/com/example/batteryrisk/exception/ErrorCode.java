...
public enum ErrorCode {
    ...
    CONSENT_REQUIRED(HttpStatus.BAD_REQUEST, "개인정보 수집·이용(필수) 동의가 필요합니다."),
    ACCOUNT_LOCKED(HttpStatus.LOCKED,
            "일정 횟수 이상 로그인에 실패하였습니다. 잠시 후 다시 시도하거나 비밀번호 재설정 이후 진행 가능합니다."),
    PASSWORD_EXPIRED(HttpStatus.FORBIDDEN, "비밀번호 유효기간(90일)이 만료되었습니다. 비밀번호 재설정 후 이용해 주세요."),
    PASSWORD_REUSED(HttpStatus.BAD_REQUEST, "직전에 사용한 비밀번호는 다시 사용할 수 없습니다."),
    CAPTCHA_REQUIRED(HttpStatus.PRECONDITION_REQUIRED, "자동입력 방지 문자를 입력해 주세요."),
    CAPTCHA_INVALID(HttpStatus.BAD_REQUEST, "자동입력 방지 문자가 일치하지 않습니다."),
