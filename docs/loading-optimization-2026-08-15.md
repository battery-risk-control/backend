...
- **C-2 보류(사용자 결정)** — 수동 `useEffect+fetch` 화면들의 useQuery 전면 이관. Part A로 API가 수십 ms가 되어
  체감 이득이 급감했으므로 우선순위 낮음. 착수 시 기능 보존 규칙(부수효과 이관·invalidateQueries 치환·업로드 후 신선도)은
  본 문서 §5 원계획 참조.
...
## 5. 커밋 목록 (perf/loading-speed)

**backend_merge**
- `perf: 뉴스 조회 생성 컬럼 최적화 + JSON gzip 압축` (V38 + 쿼리 교체 + 가드 테스트 + E-1)
- `feat(auth): 시연용 마스터 계정 - 1·2·3계층 대시보드 전체 열람` (V39 — 성능과 무관, 시연 편의)

**frontend**
- `perf: 코드 스플리팅 + 쿼리 캐시 + 청크 프리로드로 로딩 단축` (B + C-1 + E-2)
- `feat(auth): 시연용 마스터 계정 …` ×2 (전 계층 접근 + 헤더 계층 전환 탭)
- `ci: S3 업로드에 캐시 헤더 추가` (§3 빈틈 수정)
- `fix: 관리자 가입 관리 표 가로 스크롤 제거`

## 6. 남은 개선 후보 (미착수 — 배포 후 실측으로 우선순위 결정)

현 시점 기준 아직 하지 않았고, 효과가 있을 수 있는 항목. **배포 후 Network 탭 실측으로 병목이 확인될 때만 착수**한다
(Part A 이후 대부분의 API가 수십 ms라, 실측 없이 미리 고치는 것은 과잉).
...
- **뉴스 데이터 정지**: DB 최신 뉴스가 2026-08-11로, 노출 만료 규칙(심각 10일·주의 5일·그 외 3일)에 걸려
  목록이 placeholder 폴백 상태. 수집 스케줄러(`COLLECTION_SCHEDULER_ENABLED`, 기본 off) 기동 여부는 운영 판단.
