...
```
{
  ...
  "data": {
    ...
    "detectedAt": "2026-07-20T08:45:00+09:00"
  },
...
- Alias Generator로 `camelCase` 직렬화
...
1. RDB에 Contract 생성
2. contractId 발급
3. Spring Boot가 FastAPI에 PDF와 contractId 업로드
...
API 및 DB 명명 규칙에 맞춰 `camelCase` 기준으로 정의한다.
...
- `contractId` 또는 `supplierId` 중 하나는 필수
...
- `contractId` 또는 `supplierId` 필터 검색 성공
