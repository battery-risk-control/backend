"""
사전 필터(triage) 추론 모듈. (realtime-pipeline 브랜치에서 포팅)

models/triage_filter.json(부스터) + models/triage_filter_meta.json(임계값/카테고리 사전)을
로드해서, "이 이벤트를 크롤링할 가치가 있는가"를 판단하는 should_crawl()만 제공한다.
...
MODEL_PATH = os.path.join(os.path.dirname(__file__), "triage_filter.json")
META_PATH = os.path.join(os.path.dirname(__file__), "triage_filter_meta.json")

# 배터리 5대 원자재 핵심 생산국 (FIPS 10-4 코드)
